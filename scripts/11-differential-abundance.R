...
output_dirs <- c("results",
                 "results/11-differential-abundance")
...
prop_by_sample$genotype[str_detect(prop_by_sample$sample, "het")] <- "DREADD"
prop_by_sample$genotype[str_detect(prop_by_sample$sample, "wt")] <- "Control"
write.csv(prop_by_sample,
          file = "results/11-differential-abundance/prop_by_sample.csv",
          row.names = F)
...
pdf(file = "results/11-differential-abundance/milo_Nhood_size.pdf",
    useDingbats = F)
print(p)
dev.off()
...
pdf(file = "results/11-differential-abundance/milo_pvalue_spatialfdr.pdf",
    useDingbats = F)
print(p1 + p2)
dev.off()
...
pdf(file = "results/11-differential-abundance/milo_UMAP_NhoodGraph.pdf",
    height = 6.5,
    width = 12,
    useDingbats = F
    )
...
write.csv(milo_res,
          file = "results/11-differential-abundance/milo_res.csv",
          row.names = F)

# Save milo object
save(obj_milo, file = "results/0-objects/obj_milo.Rdata")
...
write.csv(topTags(ocsa_da_res, n = 34),
          file = "results/11-differential-abundance/ocsa_da_res.csv")
write.csv(topTags(ocsa_da_res_norm, n = 34),
          file = "results/11-differential-abundance/ocsa_da_res_norm.csv")

# Save sce object
save(obj_sce, file = "results/0-objects/obj_sce.Rdata")
