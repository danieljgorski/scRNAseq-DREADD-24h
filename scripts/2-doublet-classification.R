...
output_dirs <- c("results",
                 "results/2-doublet-classification")
...
# Load object
load("results/0-objects/obj_preprocessed.Rdata")
...
for (i in unique(obj@meta.data$sample)) {
  ...
  # BCmvn distributions
  pdf(file = paste0("results/2-doublet-classification/BCmvn_distributions_",
                    i,
                    ".pdf"),
      useDingbats = F)
  plot(x = pK,
  ...
  pdf(file = paste0("results/2-doublet-classification/DimPlot_classification_",
                    i,
                    ".pdf"),
      useDingbats = F)
  print(p)
  ...
  v <- VlnPlot(obj_sub,
    ...
    ggtitle(i) +
    NoLegend()
  pdf(file = paste0("results/2-doublet-classification/VlnPlot_nFeature_RNA_classification_",
                    i,
                    ".pdf"),
      useDingbats = F)
  print(v)
...
obj <- merge(x = `555-1-hM4Di-het`, y = c(`555-10-hM4Di-het`,
                                  `555-11-hM4Di-het`,
                                  `555-12-hM4Di-wt`,
                                  `555-2-hM4Di-wt`,
                                  `555-3-hM4Di-het`,
...
write.csv(barcode_classification,
          file = "results/2-doublet-classification/barcode_classification.csv",
          row.names = F)
...
write.csv(doublet_summary,
          file = "results/2-doublet-classification/doublet_summary.csv",
          row.names = F)
...
write.csv(doublet_summary_by_sample,
          file = "results/2-doublet-classification/doublet_summary_by_sample.csv",
          row.names = F)

# Save object
save(obj, file = "results/0-objects/obj_db_classified.Rdata")
