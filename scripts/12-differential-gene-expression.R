...
output_dirs <- c("results",
                 "results/12-differential-gene-expression")
...
write.csv(dge_no_threshold,
          file = "results/12-differential-gene-expression/dge_no_threshold.csv",
          row.names = F)
...
write.csv(dge,
          file = "results/12-differential-gene-expression/dge.csv",
          row.names = F)
...
p <- dge %>%
  ...
  coord_flip() +
  scale_fill_manual(name = "Regulation in DREADD",
    values = c("#D75438", "#4878CD")) +
  theme_bw() +
  ggtitle("Number of differentially expressed genes")
pdf(file = "results/12-differential-gene-expression/dge_count.pdf",
    useDingbats = F)
print(p)
dev.off()
